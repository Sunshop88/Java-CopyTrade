...
public interface FollowTraderAnalysisDao extends BaseDao<FollowTraderAnalysisEntity> {
    ...
    @Select({
            ...
            "(SELECT COUNT(1) FROM (SELECT account FROM follow_trader_analysis WHERE type=0   GROUP BY account) a) as sourceActiveNum,",
            "(SELECT COUNT(1) FROM (SELECT account FROM follow_trader_analysis WHERE type=1   GROUP BY account) a) as followActiveNum,",
