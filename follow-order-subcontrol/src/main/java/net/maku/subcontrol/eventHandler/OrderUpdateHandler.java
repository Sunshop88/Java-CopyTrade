...
public class OrderUpdateHandler implements OrderUpdateEventHandler {
    ...
    protected void send2Copiers(OrderChangeTypeEnum type, online.mtapi.mt4.Order order, double equity, String currency, LocalDateTime detectedDate) {
        ...
        // #84 喊单者发送订单前需要处理前后缀
        EaOrderInfo orderInfo = new EaOrderInfo(order, leader.getId().toString(), leader.getAccount(), leader.getServerName(), equity, currency, Boolean.FALSE);
        assembleOrderInfo(type, orderInfo, detectedDate);
        //升级前后缀删除和强制映射，前后缀信息从账号信息中获取。
        orderInfo.removeSymbolPrefixSuffix(abstractApiTrader.getPrefixSuffixList(), abstractApiTrader.getCorrectSymbolMap());
